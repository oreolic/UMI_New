...
class RPM:
    ...
    def Count_DF(self,Data_combination):
        ...
        RPMdf = DF.drop(columns=['D10_Count', 'D24_Count','Fold_Change'])
        return RPMdf
    ...
    def _Multi_Divide_DFNT(self,DF,NT):
        ...
        DFlist = [i for i in DFlist if len(i)>5]
...
class withoutD24:
    ...
    def Count_DF(self,Data_combination):
        DF = pd.read_csv('Result/{}_{}_Remove_D24_totalRPM_FC_Result.txt'.format(Data_combination[0][0], Data_combination[0][1]), sep='\t')
        RPMdf = DF.drop(columns=['D10_Count', 'D24_Count','Fold_Change'])
        return RPMdf
    ...
    def _Re_indexing(self,DivideDF,Data_combination):
        ...
        NTfinal.to_csv('Mageck/{}_{}_Remove_D24_Total_RPM_nonTarget.txt'.format(tup[0],tup[1]),header=None, index=None)
        FinalDF.to_csv('Mageck/{}_{}_Remove_D24_Total_RPM_Mageck_Input.txt'.format(tup[0],tup[1]),sep='\t', index=None)
        return 
